import { ApiSpec, Endpoint, Parameter, Schema } from './parser';
import { Change, BreakingChange, ChangeSeverity } from './detector';
  ...
  for (const [propName, oldProp] of oldProps) {
    if (!newProps.has(propName)) {
      changes.push({
        ...
        oldValue: JSON.stringify(oldProp),
      });
    }
  }
  ...
  for (const [propName, newProp] of newProps) {
    if (!oldProps.has(propName)) {
      ...
      changes.push({
        ...
        newValue: JSON.stringify(newProp),
      });
    }
  }
