...
function compareEndpoints(oldEndpoints, newEndpoints, summary) {
    const changes = [];
...
function compareEndpointDetails(oldEndpoint, newEndpoint, summary) {
    const changes = [];
    ...
    const paramChanges = compareParameters(oldEndpoint.parameters, newEndpoint.parameters, basePath, summary);
    changes.push(...paramChanges);
    if (oldEndpoint.requestBody || newEndpoint.requestBody) {
        const reqBodyChanges = compareRequestBody(oldEndpoint.requestBody, newEndpoint.requestBody, basePath);
...
function compareParameters(oldParams, newParams, basePath, summary) {
    const changes = [];
    ...
    for (const [key, oldParam] of oldMap) {
        if (!newMap.has(key)) {
            ...
            const severity = oldParam.required ? 'BREAKING' : 'WARNING';
            changes.push({
                type: 'REMOVED',
                category: 'PARAMETER',
                severity,
    ...
    for (const [key, newParam] of newMap) {
        if (!oldMap.has(key)) {
            ...
            const severity = newParam.required ? 'BREAKING' : 'INFO';
            changes.push({
                type: 'ADDED',
                category: 'PARAMETER',
                severity,
...
function compareParameterDetails(oldParam, newParam, basePath) {
    const changes = [];
...
function compareRequestBody(oldBody, newBody, basePath) {
    const changes = [];
    if (!oldBody && newBody) {
        changes.push({
            type: 'ADDED',
...
function compareResponses(oldResponses, newResponses, basePath) {
    const changes = [];
...
function compareSchemas(oldSchemas, newSchemas, summary) {
    const changes = [];
...
function compareSchemaDetails(oldSchema, newSchema) {
    const changes = [];
    ...
    for (const [propName, newProp] of newProps) {
        if (!oldProps.has(propName)) {
            const severity = newProp.required ? 'WARNING' : 'INFO';
            changes.push({
                type: 'ADDED',
                category: 'SCHEMA_PROPERTY',
                severity,
...
function createBreakingChange(change) {
    return {
        ...change,
...
function generateMigrationSuggestion(change) {
...
function calculateRiskScore(breakingChanges, allChanges) {
...
function getSemverRecommendation(breakingChanges, changes) {
